...
void walk_m2l_tasknum(int im, int jm)
{
	...
	//printf(" im = %d %d, last_leaf=%d, first_node=%d\n", im, jm,last_leaf,first_node);
	...
	if ( im == jm ) {
		...
		if ( im >= first_node ) 
		{
			//printf("  - - %d - - %d %d - - \n", im, btree[im].son[0], btree[im].son[1]);
			walk_m2l_tasknum(btree[im].son[0], btree[jm].son[0]);
			walk_m2l_tasknum(btree[im].son[0], btree[jm].son[1]);
			walk_m2l_tasknum(btree[im].son[1], btree[jm].son[0]);
			walk_m2l_tasknum(btree[im].son[1], btree[jm].son[1]);
		}
...
//			idxM2L ++;
		...
		//	printf(" m2l : %d -> %d\n", jm, im);
			...
			walk_m2l_tasknum(im, btree[jm].son[0]);
			walk_m2l_tasknum(im, btree[jm].son[1]);
		}
		else if ( -1 == flag){
			return;
		}
...
//			idxM2L ++;
		...
		m2l_count++;
			...
			walk_m2l_tasknum(btree[im].son[0], jm);
			walk_m2l_tasknum(btree[im].son[1], jm);
		}
		else if ( -1 == flag){
			return;
		}
...
{
		//	if (PROC_RANK  == 11) printf("lxj-im=%d,jm=%d,first_leaf,last_leaf=%d,%d,first_node=%d,last_node=%d\n",im,jm,first_leaf,last_leaf,first_node,last_node);
	if ( -1 == im  || -1 == jm)
		return;
	...
	if ( im == jm ) {
		...
		{	
#ifndef P2POFF
//			if (PROC_RANK  == 11) printf("lxj-idxP2P=%d\n",idxP2P);
//			double t0 = dtime();

			taskP2Ps[idxP2P] = im;
			taskP2Pt[idxP2P] = jm;
			idxP2P ++;
			if(idxP2P>=(INT64)(NPART_MEAN*500.0)){
				printf("lxj-idxP2P=%d,NTASKP2P=%d\n",idxP2P,(INT64)(NPART_MEAN*500.0));
				exit(0);
			}
...
//			dtime_p2p += dtime() - t0;
		...
		if ( im >= first_node ) 
		{
//			if (PROC_RANK  == 11) printf("1111\n");
			//printf("  - - %d - - %d %d - - \n", im, btree[im].son[0], btree[im].son[1]);
			walk_m2l_task(btree[im].son[0], btree[jm].son[0]);
			walk_m2l_task(btree[im].son[0], btree[jm].son[1]);
			walk_m2l_task(btree[im].son[1], btree[jm].son[0]);
			walk_m2l_task(btree[im].son[1], btree[jm].son[1]);
		}
...
#ifndef P2POFF
//		double t0 = dtime();
//			if (PROC_RANK  == 11) printf("lxj-1-idxP2P=%d\n",idxP2P);
...
//		dtime_p2p += dtime() - t0;
	...
	if ( im < first_node && jm >= first_node ) 
	{
		...
		int flag = acceptance(leaf[im].width, btree[jm].width, dist);
//			if (PROC_RANK  == 11) printf("2222\n");

		if ( 1 == flag ) {
//			m2l(dx, dy, dz, btree[jm].M, leaf[im].L);
//			if (PROC_RANK  == 11) printf("lxj-idxM2L=%d\n",idxM2L);

			taskM2Ls[idxM2L] = jm;
			taskM2Lt[idxM2L] = im;
			idxM2L ++;
			if(idxM2L>=(INT64)((last_node - first_node)*400.0)){
				printf("lxj-idxM2L=%d,NTASKM2L=%d\n",idxM2L,(INT64)((last_node - first_node)*400.0));
				exit(0);
			}
		...
		//	printf(" m2l : %d -> %d\n", jm, im);
			...
			walk_m2l_task(im, btree[jm].son[0]);
			walk_m2l_task(im, btree[jm].son[1]);
		}
		else if ( -1 == flag){
			return;
		}
	...
	if ( im >= first_node && jm < first_node ) {
		...
		int flag = acceptance(leaf[jm].width, btree[im].width, dist);
//			if (PROC_RANK  == 11) printf("3333\n");

		if ( 1 == flag ) {
//			m2l(dx, dy, dz, leaf[jm].M, btree[im].L);
//			if (PROC_RANK  == 11) printf("lxj-2-idxM2L=%d\n",idxM2L);
		...
		m2l_count++;
			...
			walk_m2l_task(btree[im].son[0], jm);
			walk_m2l_task(btree[im].son[1], jm);
		}
		else if ( -1 == flag){
			return;
		}
	...
	if ( im >= first_node && jm >= first_node ) {
		...
		int flag = acceptance(btree[im].width, btree[jm].width, dist);
//			if (PROC_RANK  == 11) printf("4444\n");

		if ( 1 == flag ) {
//			m2l(dx, dy, dz, btree[jm].M, btree[im].L);

//			if (PROC_RANK  == 11) printf("lxj-3-idxM2L=%d\n",idxM2L);
...
void walk_m2l(int im, int jm)
{
	...
	if ( im == jm ) {
		...
		{
			//printf("  - - %d - - %d %d - - \n", im, btree[im].son[0], btree[im].son[1]);
			walk_m2l(btree[im].son[0], btree[jm].son[0]);
			walk_m2l(btree[im].son[0], btree[jm].son[1]);
			walk_m2l(btree[im].son[1], btree[jm].son[0]);
			walk_m2l(btree[im].son[1], btree[jm].son[1]);
		}
	...
	if ( im < first_node && jm >= first_node ) 
	{
		...
		wi = leaf[im].width[0];
		if (leaf[im].width[1] > wi)
			wi = leaf[im].width[1];
		if (leaf[im].width[2] > wi)
			wi = leaf[im].width[2];


		wj = btree[jm].width[0];

		if (btree[jm].width[1] > wj)
			wj = btree[jm].width[1];

		if (btree[jm].width[2] > wj)
			wj = btree[jm].width[2];


		if ( wi+wj < open_angle*dr ) {
			m2l(dx, dy, dz, btree[jm].M, leaf[im].L);
		}
		else {
			walk_m2l(im, btree[jm].son[0]);
			walk_m2l(im, btree[jm].son[1]);
		}
	...
	if ( im >= first_node && jm < first_node ) {
		...
		wi = btree[im].width[0];
		if (btree[im].width[1] > wi)
			wi = btree[im].width[1];
		if (btree[im].width[2] > wi)
			wi = btree[im].width[2];


		wj = leaf[jm].width[0];
		if (leaf[jm].width[1] > wj)
			wj = leaf[jm].width[1];
		if (leaf[jm].width[2] > wj)
			wj = leaf[jm].width[2];


		if ( wj+wi < open_angle*dr ) {
			m2l(dx, dy, dz, leaf[jm].M, btree[im].L);
		}
		else 
		{
			walk_m2l(btree[im].son[0], jm);
			walk_m2l(btree[im].son[1], jm);
		}
		return ;
...
void showtree() {
	FILE *fd = fopen("showpact.txt","w");
	int n,m, idx;
	//	fprintf(fd, "pack   npart   idx\n");
	for (n=first_leaf; n<last_leaf; n++) {
		fprintf(fd, "%d %d %d  %lf %lf %lf %e %e %e %e\n", n, leaf[n].ipart, leaf[n].npart, leaf[n].center[0],leaf[n].center[1], leaf[n].center[2], leaf[n].M[0], leaf[n].M[1], leaf[n].M[2], leaf[n].M[3]);
	}
	fclose(fd);
	...
	fd = fopen("showtree.txt","w");
	for (n=first_leaf; n<last_leaf; n++) {
		double center[3] = {0.0, 0.0, 0.0};
		for (m=0; m<leaf[n].npart; m++) {
			idx = leaf[n].ipart + m;
			center[0] += part[idx].pos[0];
			center[1] += part[idx].pos[1];
			center[2] += part[idx].pos[2];
		}
		center[0] /= leaf[n].npart;
		center[1] /= leaf[n].npart;
		center[2] /= leaf[n].npart;


		fprintf(fd, "%d %lf %lf %lf %lf %lf %lf\n",n, leaf[n].center[0], leaf[n].center[1], leaf[n].center[2], center[0], center[1], center[2]);
