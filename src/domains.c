...
void measure_domain_runtime(double dTimeFrac) {
	...
	my_gb=my_gb+sizeof(double)*PROC_SIZE/1024/1024/1024;
...
double fraction(int size, int *l, int *r) {
	...
	//	printf("left = %d, right = %d size = %d, frac=%lf\n", left, right,size, frac);
...
void determine_split_node(int D, int nproc, int iNode, DomainNode *tree, double bl[], double br[])
{
	...
	//	shift = 0.5*(t2-t1)*(w0l+w0r)/(t1+t2);    

	//shift = 0.0;

	new_split = domtree[iNode].split + shift;

	//	printf("{%d} %lf %lf %lf\n", PROC_RANK, DTIME_FRACTION, shift, new_split);
	/*
	   if (new_split > 1.02*domtree[iNode].split)
	   new_split = 1.02*domtree[iNode].split ;
	   if (new_split < 0.98*domtree[iNode].split)
	   new_split = 0.98*domtree[iNode].split ;
	   */
...
void bksort_body_inplace(int D, Body *p, int length, int npart[2], double split)
{
	...
	//        printf(">[%d] count %d %d, D = %d split = %lf\n", PROC_RANK, count, length, D, split);
...
void prepare_deliver_realloc_body(int *npart)
{
	...
	MPI_Barrier(MPI_COMM_WORLD);
	if(PROC_RANK==0) printf("begin realloc_body\n");
	my_gb=my_gb+sizeof(int)*PROC_SIZE*4/1024/1024/1024;

	//printf("prepare_deliver_realloc_body\n");
	//fflush(stdout);
	for (n=0; n<PROC_SIZE; n++) {
		sendcount[n] = 0;
		recvcount[n] = 0;
	...
	//printf(" npart in prepare_deliver_realloc_body = %d fisrt-domain = %d\n", *npart, first_domain);
	...
	my_gb=my_gb+sizeof(Body)*new_npart/1024/1024/1024;

	//	MPI_Alltoallv(part, sendcount, senddisp, MPI_BYTE, part_b, recvcount, recvdisp, MPI_BYTE, MPI_COMM_WORLD);

//bak	MPI_Alltoallv(part, sendcount, senddisp, strBody, part_b, recvcount, recvdisp, strBody, MPI_COMM_WORLD);
//////////////////////////////////add by lxj
	int rank,nproc;
	rank=PROC_RANK;
	nproc=PROC_SIZE;
...
//bak	MPI_Alltoallv(part, sendcount, senddisp, strBody, part_b, recvcount, recvdisp, strBody, MPI_COMM_WORLD);
	...
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
	if(PROC_RANK==0) printf("end part_b\n");
//
/////////////////////////////////end add by lxj
...
void domain_decomposition() 
{
	...
	/*
	   ...
	   MPI_Allgather(&DTIME_FRACTION, 1, MPI_DOUBLE, 
	   time_rank, 1, MPI_DOUBLE, MPI_COMM_WORLD);
...
void domain_initialize() {
	...
	my_gb=my_gb+sizeof(DomainNode) * length/1024/1024/1024;
