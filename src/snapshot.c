...
//printf(" mass - heaf = %lf, %lf\n", head.mass[1], head.mass[0]);
...
void read_Particle(char filename[], int n_start, int n_count)
{
	...
	//    printf(" num_part = %lu ,%d ,%d \n", num_part, n_start, n_count);
	//    printf(" npart = %d %d \n",  head.npart[1], head.npart[2]);
	//    printf(" mass  = %lf %lf \n", head.mass[1],  head.mass[2]);

	//    disp  = (2*sizeof(dummy) + sizeof(GadgetHeader));
	//    disp += (sizeof(dummy) + n_start*3*DATA_SIZE );
	//    fseek(fsnap, disp,0);
	...
	//    disp  += (2*sizeof(dummy) + num_part*3*DATA_SIZE );
	//    fseek(fsnap, disp,0);
...
void write_snapshot(int type, int timestamp, int rank ) 
{
   ...
   // sprintf(fout, "%s_%d.%d", OutputPath, timestamp, rank);
